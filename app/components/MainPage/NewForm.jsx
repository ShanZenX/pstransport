...
export default function TaxiBookingForm() {
  ...
  const handleSubmit = async () => {
    let newErrors = {};
    if (!from && rideType !== "airport") newErrors.from = "Pickup location required";
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        const pickupLat = rideType === "airport" ? 12.9941 : from.properties.lat;
        const pickupLon = rideType === "airport" ? 80.1709 : from.properties.lon;
        ...
        if (data?.features?.length) {
          ...
          const distanceKm = parseFloat((leg.distance / 1000).toFixed(2));
  ...
  const handleConfirmBooking = () => {
    ...
    if (!passengerName.trim()) validationErrors.passengerName = "Passenger name required";
    ...
    if (Object.keys(validationErrors).length === 0) {
      const pickupName = rideType === "airport" ? "Chennai International Airport" : from.properties.formatted;
      ...
      const whatsappMsg = encodeURIComponent(
        ...
        `Fare: ₹${totalCost}\n\n` +
        `Google Maps Links:\n` +
        `Pickup: https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(pickupName)}\n` +
        `Drop: https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(dropName)}\n` +
        `Directions: https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(pickupName)}&destination=${encodeURIComponent(dropName)}`
      );
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isActive ? "border-black  text-white" : "border-transparent text-white hover:text-black"}
              `}
            >
      ...
      <div className="border-2 border-black rounded-2xl p-4 flex flex-col text-start gap-4 bg-white shadow-lg max-w-full lg:max-w-fit">
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition ${
                  isActive ? "bg-black text-white " : "bg-gray-200 text-black sm:text-black hover:bg-gray-300"
                }`}
              >
                {type.charAt(0).toUpperCase() + type.slice(1)}
              </button>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap  gap-8">
          ...
          <div className="flex mb-2 sm:mb-0  flex-col w-full sm:w-1/2 lg:w-44">
            <label className="text-sm font-semibold text-black mb-1">Departure</label>
            <input
              type="datetime-local"
              value={dateTime}
          ...
          {rideType === "twoway" && (
            <div className="flex mb-2 sm:mb-0  flex-col w-full sm:w-1/2 lg:w-44 transition-all duration-500">
              <label className="text-sm font-semibold text-black mb-1">Return</label>
              <input
                type="datetime-local"
              ...
              />
              {errors.returnDateTime && <p className="text-red-500 text-xs mt-1">{errors.returnDateTime}</p>}
            </div>
          )}
          ...
          <div className="flex mb-2 sm:mb-0  flex-row w-full gap-4 sm:flex-col sm:w-1/2 lg:flex-row lg:w-auto">
            <div className="flex flex-col w-1/2 sm:w-full lg:w-28">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (parseInt(e.target.value) > 4 && vehicle === "Sedan") setVehicle("Ertiga");
                }}
            ...
            <div className="flex mb-2 sm:mb-0  flex-col w-1/2 sm:w-full lg:w-32">
              <label className="text-sm font-semibold text-black mb-1">Car Type</label>
              <select
                value={vehicle}
              ...
              >
                {vehicleOptions.map((v) => (
                  ...
                  >
                    {v} {v === "Sedan" && passenger > 4 ? "(Not enough seats)" : ""}
                  </option>
                ))}
              </select>
            </div>
        ...
        <DialogContent className="flex flex-col gap-4 mt-2">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              <label className="text-sm font-semibold mb-1">Passenger Name</label>
              <input
                type="text"
                value={passengerName}
              ...
              {errors.passengerName && (
                <p className="text-red-500 text-xs mt-1">{errors.passengerName}</p>
              )}
            </div>
            <div className="flex-1">
          ...
          <div className="bg-gray-100 p-4 rounded-lg text-sm space-y-2">
            <p><strong>From:</strong> {rideType === "airport" ? "Chennai International Airport" : from?.properties?.formatted}</p>
            <p><strong>To:</strong> {to?.properties?.formatted}</p>
            <p><strong>Departure:</strong> {dateTime}</p>
            {rideType === "twoway" && <p><strong>Return:</strong> {returnDateTime}</p>}
            <p><strong>Trip Type:</strong> {rideType.charAt(0).toUpperCase() + rideType.slice(1)}</p>
            <p><strong>Passengers:</strong> {passenger}</p>
            <p><strong>Vehicle:</strong> {vehicle}</p>
            {distance && (
              <>
                <p><strong>Total Distance:</strong> {distance.distanceKm} km</p>
                <p><strong>Estimated Time:</strong> {distance.timeMin} min</p>
                <p className="text-lg font-bold text-green-600">Total Fare: ₹{totalCost}</p>
              </>
            )}
          </div>
        </DialogContent>
